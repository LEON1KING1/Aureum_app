// script.js - handles WebApp interactions and sends data to the bot via tg.sendData()
...
document.getElementById('mineBtn').addEventListener('click', ()=>{
  ...
  document.getElementById('mineMsg').innerText = 'Mining request sent...';
});

document.querySelectorAll('.task').forEach(t=>{
  t.addEventListener('click', ()=>{
    const id = t.dataset.id;
    if(id === 'invite'){ openInvite(); return; }
    sendToBot({action:'task', task_id: id});
    alert('Task submitted — reward will be processed by the bot.');
  });
});

function openInvite(){
  if(!tg){ alert('Open inside Telegram to use invite'); return; }
  ...
  if(!username){ alert('Set a Telegram username in your profile to use invites.'); return; }
  const invite = `https://t.me/AureumToken_bot?start=${username}`;
  sendToBot({action:'invite', ref: username});
  tg.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(invite)}&text=${encodeURIComponent('Join Aureum and earn tokens! 💎')}`);
}

function sendToBot(obj){
  if(!tg){
    alert('Open this page inside Telegram WebApp to interact with the bot.');
    return;
  }
